import { 
  ...
  users, whatsappSessions, whatsappAccounts, contactGroups, contactGroupMembers, bulkMessageCampaigns
} from "@shared/schema";
...
export interface IStorage {
  ...
  getStoredAccounts(): Promise<Array<{
    sessionId: string;
    name: string;
    phone: string;
    status: 'disconnected' | 'connecting' | 'qr_required' | 'connected';
  ...
  removeStoredAccount(sessionId: string): Promise<void>;
}
...
export class MemStorage implements IStorage {
  ...
  async saveAccountInfo(account: {
    ...
    // Since MemStorage is for testing/development, just log this
    console.log('MemStorage: saveAccountInfo called', account);
  }

  async getStoredAccounts(): Promise<Array<{
    sessionId: string;
    name: string;
    phone: string;
    status: 'disconnected' | 'connecting' | 'qr_required' | 'connected';
    ...
    // Since MemStorage is for testing/development, return empty array
    return [];
  }

  async removeStoredAccount(sessionId: string): Promise<void> {
    // Since MemStorage is for testing/development, just log this
    console.log('MemStorage: removeStoredAccount called', sessionId);
  }
}
...
export class DatabaseStorage implements IStorage {
  ...
  async saveAccountInfo(account: {
    ...
    try {
      ...
      const [updated] = await db
        ...
        .where(eq(whatsappAccounts.sessionId, account.sessionId))
        .returning();
  ...
  async getStoredAccounts(): Promise<Array<{
    ...
    return accounts.map(acc => ({
      ...
      loginTime: acc.loginTime || undefined,
      sessionData: acc.sessionData || undefined
    }));
  }
